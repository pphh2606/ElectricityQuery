 ...
 *
 * 页面自身只负责渲染与导航；数据在 `jwxt/home/data`，登录凭证与错误归类在 `jwxt/core`，
 * 状态与取数编排在 [JwxtHomeViewModel]。所有子功能点击都交给内置浏览器（本次只本地化首页）。
 *
                ...
                LazyColumn(
                        ...
                        if (uiState.isLoading) {
                            ...
                            JwxtServiceSection(
                                ...
                                when {
                                    ...
                                    item.serviceKey.startsWith(JwxtConstants.SERVICE_KEY_SCHEDULE_QUERY_PREFIX) ->
                                        nav.navigate(Routes.JWXT_SCHEDULE_QUERY)
                    ...
                    item(key = "lesson_exam") {
                        JwxtLessonExamCard(
                            ...
                            // 卡片底部的「更多课程」：进全校课表查询（与首页九宫格那个格子同一个页面）
                            onMoreLessons = { nav.navigate(Routes.JWXT_SCHEDULE_QUERY) },
                        )
                    }
                }
            }
